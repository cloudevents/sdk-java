...
public final class EventDataCodecProvider implements EventDataCodec {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public <T> T deserialize(@Nullable String dataContentType, Object data, Class<T> clazz) throws DataDeserializationException {
        ...
        EventDataCodec codec = codecs
            .stream()
            .filter(c -> c.canDeserialize(ct))
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("Cannot find any registered codec that can deserialize from " + ct));
        return codec.deserialize(ct, data, clazz);
    }
    ...
    public boolean canDeserialize(String dataContentType) {
        return codecs
            .stream()
            .anyMatch(c -> c.canDeserialize(dataContentType));
    }
